...
const UpdateProvider = ({
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(`Connection test failed`, {
          description: data.error || 'No response from provider',
          duration: 8000,
        });
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    ...
    (fields || []).forEach((field) => {
      config[field.key] =
        modelProvider?.config?.[field.key] ??
        (modelProvider as any)?.[field.key] ??
        field.default ??
        '';
    });
                ...
                <form onSubmit={handleSubmit} className="flex flex-col flex-1">
                  ...
                  <div className="px-6 py-4 flex justify-between items-center">
                    <button
                      ...
                      className="px-3.5 py-2 rounded-lg text-[13px] border border-light-200 dark:border-dark-200 text-black/80 dark:text-white/80 hover:bg-light-200 hover:dark:bg-dark-200 flex items-center space-x-1.5 active:scale-95 transition duration-200"
                    >
                      {testing ? (
