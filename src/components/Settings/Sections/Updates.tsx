...
const Updates: React.FC = () => {
  ...
  return (
    <div className="p-6 space-y-6 max-w-3xl">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-neutral-900/90 via-neutral-900/50 to-neutral-950 p-6 backdrop-blur-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2.5">
              ...
              <span className="px-2.5 py-0.5 text-xs font-mono font-semibold rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
                v{CURRENT_VERSION}
              </span>
              {hasUpdate === false && (
              ...
              {hasUpdate === true && (
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 text-xs font-medium rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20 animate-pulse">
          ...
          <button
            ...
            disabled={isChecking}
            className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 hover:border-cyan-500/50 font-medium text-xs transition duration-200 active:scale-95 disabled:opacity-50"
          >
        ...
        <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-6 space-y-4">
          ...
          {releaseInfo.body && (
            <div className="mt-4 p-4 rounded-xl bg-black/40 border border-white/5 space-y-2">
              <p className="text-xs font-semibold text-neutral-300">Changelog & Highlights:</p>
              <div className="text-xs text-neutral-400 max-h-48 overflow-y-auto whitespace-pre-wrap font-mono leading-relaxed">
                {releaseInfo.body}
              </div>
            </div>
          )}
      ...
      <div className="rounded-2xl border border-white/10 bg-neutral-900/40 p-6 space-y-4">
        <h3 className="text-sm font-semibold text-white">Update Preferences</h3>
        
        <div className="flex items-center justify-between py-2 border-b border-white/5">
          <div className="space-y-0.5">
            <label htmlFor="auto-update-toggle" className="text-xs font-medium text-neutral-200 cursor-pointer">
          ...
          <input
            ...
            className="w-4 h-4 rounded bg-neutral-800 border-white/20 text-cyan-500 focus:ring-cyan-500/30 cursor-pointer"
          />
        </div>
        ...
        <div className="flex items-center justify-between py-2">
          ...
          <span className="px-2.5 py-1 rounded-lg text-xs bg-white/5 border border-white/10 text-neutral-300 font-mono">
            Stable (Official)
          </span>
        </div>
      </div>
      ...
      {releaseInfo && releaseInfo.assets && releaseInfo.assets.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-neutral-900/40 p-6 space-y-4">
          ...
          >
            ...
            <div className="flex items-center gap-1 text-xs text-neutral-400 hover:text-white">
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {isRecommended && (
                            <span className="px-1.5 py-0.2 text-[10px] rounded bg-cyan-500/20 text-cyan-300 font-medium">
                              Your OS
                            </span>
                          )}
                        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between p-4 rounded-xl border border-white/5 bg-white/[0.02] gap-3 text-xs text-neutral-400">
        <div className="flex items-center gap-2">
