...
/// Automatically loads the active API key generated by 9Router from its local SQLite database
pub fn get_9router_api_key() -> Option<String> {
    ...
    let base_dir = std::env::var("APPDATA")
        ...
        .or_else(|| {
            std::env::var("USERPROFILE")
                .ok()
                .map(|u| std::path::PathBuf::from(u).join("AppData").join("Roaming"))
        })
        .map(|p| p.join("9router"));

    #[cfg(not(target_os = "windows"))]
    let base_dir = std::env::var("HOME")
        .ok()
        .map(|h| std::path::PathBuf::from(h).join(".9router"));
    ...
    pub async fn complete(
        ...
        match provider_type {
            ...
            _ => {
                ...
                let auto_key = if api_key.map(|s| s.trim().is_empty()).unwrap_or(true) {
                    if provider_type == "9router" || effective_base.contains("20128") {
                        get_9router_api_key()
                    } else {
                        None
                    }
